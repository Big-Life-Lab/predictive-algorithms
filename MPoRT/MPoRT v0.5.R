...
data$dep <- as.factor(sample(c('dep1', 'dep2', 'dep3'), 5000, replace=TRUE, prob=c(0.3, 0.4, 0.3)))
...
##Demographics##
#Sex
databox <- FunctionXform(databox, origFieldName= "sex", newFieldName= "Sex", formulaText= "if(sex=='fem'){1} else if(sex=='male'){0}")

#Age
databox <- RenameVar(databox, "age->Age_cont")
                     
#Age Spline
databox <- FunctionXform(databox, origFieldName= c("sex", "age"), newFieldName= "Age_spline", formulaText= "if(Sex=='fem' && Age>80){Age-80} else if(Sex=='fem'){0} else if(Sex=='male' && Age>65){Age-65} else {0}")

#Education
databox <- FunctionXform(databox, origFieldName= c("hs", "ed"), newFieldName= "EduNoGrad_cat", formulaText= "if(hs=='hs2' && ed=='ed2'){1} else {0}")
databox <- FunctionXform(databox, origFieldName= c("hs", "ed", "hdg"), newFieldName= "EduHSGrad_cat", formulaText= "if((hs=='hs1' && ed=='ed2') | (hs=='hs1' && ed=='ed1' && hdg=='hdg1')){1} else {0}")

#Deprivation
databox <-FunctionXform(databox, origFieldName= "dep", newFieldName= "DepIndHigh_cat", formulaText="if(dep=='dep1'){1} else {0}")
databox <-FunctionXform(databox, origFieldName= "dep", newFieldName= "DepIndMod_cat", formulaText="if(dep=='dep2'){1} else {0}")

#Immigration
databox <- FunctionXform(databox, origFieldName= c("immm", "imyr"), newFieldName= "ImEth0To15_cat", formulaText= "if(imm=='imm2' && imyr>=1 && imyr<=15){1} else if(imm=='imm1'|imyr>15){0} else {0}")
databox <- FunctionXform(databox, origFieldName= c("immm", "imyr"), newFieldName= "ImEth16To30_cat", formulaText= "if(imm=='imm2' && imyr>15 && imyr<=30){1} else if(imm=='imm1' | imyr<=15 | imyr>30){0} else {0}")
databox <- FunctionXform(databox, origFieldName= c("immm", "imyr"), newFieldName= "ImEth31To45_cat", formulaText= "if(imm=='imm2' && imyr>30 && imyr<=45){1} else if(imm=='imm1'|imyr<=30|imyr>45){0} else {0}")
...
databox <- FunctionXform(databox, origFieldName= c("smk_lightraw", "sex", "quittime"), newFieldName= "QSLight_df", formulaText= "if(formerlightflag==1 && sex=='fem'){exp(quittime/26)} else if(formerlightflag==1 && sex=='male'){exp(quittime/15)} else {smk_lightraw}")
databox <- FunctionXform(databox, origFieldName= c("smk_heavyraw", "sex", "quittime"), newFieldName= "QSHeavy_df", formulaText= "if(formerheavyflag==1 && sex=='fem'){exp(quittime/26)} else if(formerheavyflag==1 && sex=='male'){exp(quittime/15)} else {smk_heavyraw}")
...
databox <- FunctionXform(databox, origFieldName= c("dev", "bingeflag", "dany", "weeklyalc", "db", "Sex"), newFieldName= "AlcoholMod_cat", formulaText= "if(!is.na(bingeflag) && bingeflag==1){0} else if(Sex==0 && dev=='dev1' && dany=='dany1' && weeklyalc>3 && weeklyalc <=21){1} else if(Sex==1 && dev=='dev1' && dany=='dany1' && weeklyalc>2 && weeklyalc <=14){1} else if(Sex==0 && dev=='dev1' && dany=='dany1' && weeklyalc<=3){0} else if(Sex==1 && dev=='dev1' && dany=='dany1' && weeklyalc<=2){0} else if(Sex==0 && dev=='dev1' && dany=='dany1' && weeklyalc>21){0} else if(Sex==1 && dev=='dev1' && dany=='dany1' && weeklyalc>14){0} else if(dev=='dev2' | dany=='dany2'){0} else {NA}")
databox <- FunctionXform(databox, origFieldName= c("dev", "devf", "dany", "weeklyalc", "bingeflag", "Sex"), newFieldName= "AlcoholHeavy_cat", formulaText= "if((!is.na(bingeflag) && bingeflag==1) | (Sex==0 && dev=='dev1' && dany=='dany1' && weeklyalc>21)){1} else if(Sex==1 && dev=='dev1' && dany=='dany1' && weeklyalc>14){1} else if(Sex==0 && dev=='dev1' && dany=='dany1' && weeklyalc<=21){0} else if(Sex==1 && dev=='dev1' && dany=='dany1' && weeklyalc<=14){0} else if(dev=='dev2' | dany=='dany2'){0} else {NA}")
...
databox <- FunctionXform(databox, origFieldName=c('dpotato'), newFieldName= "highpotatoflag", formulaText= "if(Sex==0 && (dpotato*7)>=7){1} else if(Sex==0 && (dpotato*7)>=5){1} else {0}")
...
databox <- FunctionXform(databox, origFieldName="dietraw", newFieldName= "DietScore_cont", formulaText= "if(dietraw<0){0} else if(dietraw>10){10} else {dietraw}")

##Chronic Conditions##
#Heart Disease
databox <- FunctionXform(databox, origFieldName= "hd", newFieldName= "HeartDis_cat", formulaText= "if(hd=='hd1'){1} else if(hd=='hd2'){0} else {NA}")
#Diabetes
databox <- FunctionXform(databox, origFieldName= "diab", newFieldName= "Diabetes_cat", formulaText= "if(diab=='diab1'){1} else if(diab=='diab2'){0} else {NA}")
#Stroke
databox <- FunctionXform(databox, origFieldName= "stk", newFieldName= "Stroke_cat", formulaText= "if(stk=='stk1'){1} else if(stk=='stk2'){0} else {NA}")
#Cancer
databox <- FunctionXform(databox, origFieldName= "can", newFieldName= "Cancer_cat", formulaText= "if(can=='can1'){1} else if(can=='can2'){0} else {NA}")

#Diabetes Age Interaction
databox <- FunctionXform(databox, origFieldName= c("Diabetes_cat", "Age"), newFieldName= "DiabetesAge_int", formulaText= "{Diabetes_cat*Age_cont}")
#Cancer Age Interaction
databox <- FunctionXform(databox, origFieldName= c("Cancer_cat", "Age"), newFieldName= "CancerAge_int", formulaText= "{Cancer_cat*Age_cont}")
...
databox <- FunctionXform(databox, origFieldName="weightkg, heightm", newFieldName= "BMI_spline", formulaText= "if((weightkg/(heightm*heightm)-35)<=0){0} else if((weightkg/(heightm*heightm)-35)>0){(weightkg/(heightm*heightm))-35}")
...
#Model
model <- coxph(Surv(time, death) ~ Age_cont + Age_spline + EduHSGrad_cat + EduNoGrad_cat + ImEth015_cat + ImEth1630_cat + ImEth3145_cat 
               + DepIndMod_cat + DepIndHigh_cat + QSLight_df + QSHeavy_df + PhysicalActivity_cont + DietScore_cont + HeartDis_cat + Stroke_cat + BMI_cont + Cancer_cat 
