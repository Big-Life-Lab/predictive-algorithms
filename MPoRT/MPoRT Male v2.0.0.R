...
data <- data.frame(cbind(time, StartYear, bdate, a1, heightin_hft, heightin_hin, weightlb, hs, ed, hdg))
...
data$imyr <- sample(c("1960", "1965", "1968", "1970", "1974", "1981", "1982", "1990", "1991", "1994", "1997", "1999", "2000", "2001", "2004", "2006", "2008", "2009", "2013", "2016"), 1000, replace=TRUE, prob=c(.05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05, .05))
...
data$drk_drkm[data$dany!='dany1']<-NA
...
data$drk_drkt[data$dany!='dany1']<- NA
...
data$drk_drkw[data$dany!='dany1'] <- NA
...
data$drk_drksu[data$dany!='dany1'] <- NA
...
#Age Spline#
databox <- FunctionXform(databox, origFieldName= "Age_cont",
                         newFieldName= "Age_spline",
                         formulaText= "if(is.na(Age_cont)) {NA} else if(Age_cont>65){Age_cont-65} else {0}",
                         mapMissingTo= NA)
...
databox <- FunctionXform(databox, origFieldName= c("stpn", "stpny", "missingsmoking"),
                         ...
                         mapMissingTo= NA)
databox <- FunctionXform(databox, origFieldName= c("smklight", "formerlight", "quittime", "missingsmoking"),
                         newFieldName= "QSLight_df",
                         formulaText= "if(missingsmoking==1) {NA} else if(smklight==1) {1} else if(formerlight==1) {exp(-quittime/15)} else {0}",
                         mapMissingTo = NA)
databox <- FunctionXform(databox, origFieldName= c("smkheavy", "formerheavy", "quittime", "missingsmoking"),
                         newFieldName= "QSHeavy_df",
                         formulaText= "if(missingsmoking==1) {NA} else if(smkheavy==1) {1} else if(formerheavy==1) {exp(-quittime/15)} else {0}",
                         mapMissingTo = NA)             
...
databox <- FunctionXform(databox,
                         ...
                         formulaText= "if(missingalcohol==1) {NA} else if(bingeflag==1){0} else if(dev=='dev1' && dany=='dany1' && weeklyalc>3 && weeklyalc <=21){1} else {0}",
                         mapMissingTo = NA)
databox <- FunctionXform(databox,
                         origFieldName= c("dev", "dany", "weeklyalc", "bingeflag", "missingalcohol"),
                         newFieldName= "AlcoholHeavy_cat",
                         formulaText= "if(missingalcohol==1) {NA} else if(bingeflag==1){1} else if(dev=='dev1' && dany=='dany1' && weeklyalc>21){1} else {0}",
                         mapMissingTo = NA)
...
databox <- FunctionXform(databox,
                         ...
                         formulaText= "if(missingdiet==1) {NA} else if((dpotato*7)>=7){1} else {0}",
                         mapMissingTo = NA)
databox <- FunctionXform(databox,
...
databox <- FunctionXform(databox, origFieldName= c("lpa_lpa14", "lpat_lpa14", "lpam_lpa14"),
...
#kable(head(databox$data,200))

save(databox, file="MPoRTMaleDatabox.rda")
...
#Transformations
sink("MPoRT Male Transformation Output.txt")
output1 <- pmml(, transform=databox)
print(output1)
sink()
file.show("MPoRT Male Transformation Output.txt")
save(output1.txt)
